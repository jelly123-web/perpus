...
<div
    class="member-page"
    id="asyncBooksPage"
    data-books-notification-signatures="<?php echo e($procurementSuggestions->whereIn('status', ['approved', 'rejected'])->map(fn ($procurement) => 'superadmin-procurement-'.$procurement->id.'-'.$procurement->status.'-'.(optional($procurement->rejected_at ?? $procurement->approved_at)?->timestamp ?? optional($procurement->updated_at)->timestamp ?? 0))->implode('|')); ?>"
>
    ...
    <section id="bookStatsWrap" class="member-mini-stats">
        <div class="member-mini-stat">
            ...
            <div><div class="member-mini-value"><?php echo e($bookStats['total']); ?></div><div class="member-mini-label">Total Buku</div></div>
        </div>
        <div class="member-mini-stat">
            <div class="member-mini-icon" style="background:var(--gold-light);color:var(--gold);"><i data-lucide="package" class="w-4 h-4"></i></div>
            <div><div class="member-mini-value"><?php echo e($bookStats['stock_total']); ?></div><div class="member-mini-label">Total Stok Buku</div></div>
        </div>
        <div class="member-mini-stat">
            ...
            <div><div class="member-mini-value"><?php echo e($bookStats['stock_available']); ?></div><div class="member-mini-label">Stok Tersedia</div></div>
        </div>
        <div class="member-mini-stat">
            <div class="member-mini-icon" style="background:var(--orange-light);color:var(--orange);"><i data-lucide="tags" class="w-4 h-4"></i></div>
            <div><div class="member-mini-value"><?php echo e($bookStats['categories']); ?></div><div class="member-mini-label">Kategori Aktif</div></div>
        </div>
    </section>
    ...
    <div class="book-shell">
        <div class="book-add">
            ...
            <div class="book-category-box book-category-box--utility" style="margin-top:18px;margin-bottom:18px;">
                ...
                <form method="POST" action="<?php echo e(route('admin.books.import')); ?>" enctype="multipart/form-data" class="space-y-3" data-async="true" data-reset-on-success="true" data-refresh-targets="#bookStatsWrap,#bookListWrap">
                    <?php echo csrf_field(); ?>
                ...
                <a href="<?php echo e(route('admin.books.export')); ?>" class="btn-book-glow w-full py-3 rounded-xl font-bold mt-3" style="display:flex;">Backup Buku</a>
            </div>

            <form id="createBookForm" method="POST" action="<?php echo e(route('admin.books.store')); ?>" enctype="multipart/form-data" class="space-y-3 mt-5" data-async="true" data-reset-on-success="true" data-success-call="resetCreateBookFormState" data-refresh-targets="#bookStatsWrap,#bookListWrap">
                <?php echo csrf_field(); ?>
                ...
                <div class="book-grid">
                    <input name="published_year" type="number" min="1901" max="<?php echo e(now()->addYear()->format('Y')); ?>" class="form-input px-3 py-3 text-sm" placeholder="Tahun terbit">
            ...
            <div class="book-category-box" style="margin-top:18px;">
                ...
                <form method="POST" action="<?php echo e(route('admin.books.procurements.store')); ?>" class="space-y-4" data-async="true" data-reset-on-success="true" data-refresh-targets="#procurementListWrap">
                    <?php echo csrf_field(); ?>
                    ...
                    <div class="book-row">
                        <div class="book-row-main">
                            <?php if($book->cover_image): ?>
                                <img src="<?php echo e(asset('storage/'.$book->cover_image)); ?>" alt="<?php echo e($book->title); ?>" class="book-cover-chip" style="object-fit:cover;">
                            <?php else: ?>
                                <div class="book-cover-chip"><?php echo e(strtoupper(substr($book->title, 0, 1))); ?></div>
                            <?php endif; ?>
                            <div class="book-row-meta">
                                <div class="book-row-title"><?php echo e($book->title); ?></div>
                                <div class="book-row-sub"><?php echo e($book->author); ?></div>
                                <div class="book-row-sub2">
                                    Stok <?php echo e($book->stock_available); ?>/<?php echo e($book->stock_total); ?> | ISBN <?php echo e($book->isbn ?: '-'); ?>

                                    <?php if($book->status !== 'available'): ?>
                                        | <span class="text-red-600 font-bold"><?php echo e(strtoupper($book->status)); ?></span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                        <div class="book-actions">
                            <button
                                ...
                                data-place="<?php echo e($book->place_of_publication); ?>"
                                data-isbn="<?php echo e($book->isbn); ?>"
                                data-published-year="<?php echo e($book->published_year); ?>"
                                data-stock-total="<?php echo e($book->stock_total); ?>"
                                data-stock-available="<?php echo e($book->stock_available); ?>"
                                data-status="<?php echo e($book->status); ?>"
                                data-category-id="<?php echo e($book->category_id); ?>"
                                data-description="<?php echo e($book->description); ?>"
                                data-cover-url="<?php echo e($book->cover_image ? asset('storage/'.$book->cover_image) : ''); ?>"
                            >
                                <i data-lucide="edit-3" class="w-4 h-4"></i> Edit
                            </button>
                            <form method="POST" action="<?php echo e(route('admin.books.destroy', $book)); ?>" data-async="true" data-confirm="Hapus buku ini?" data-remove-closest=".book-row" data-refresh-targets="#bookStatsWrap,#bookListWrap">
                                <?php echo csrf_field(); ?>
    ...
    <div id="procurementListWrap" class="book-card" style="margin-top:20px;">
        ...
        <?php if($procurementSuggestions->count()): ?>
            <?php $__currentLoopData = $procurementSuggestions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $procurement): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <div class="book-row">
                    <div class="book-row-main">
                        <div class="book-cover-chip"><i data-lucide="clipboard-plus" class="w-5 h-5"></i></div>
                        <div class="book-row-meta">
                            <div class="book-row-title"><?php echo e($procurement->title); ?></div>
                            <div class="book-row-sub"><?php echo e($procurement->author); ?> | Jumlah usulan <?php echo e($procurement->quantity); ?></div>
                            <div class="book-row-sub2">
                                Status
                                <?php echo e($procurement->status === 'approved' ? 'Disetujui' : ($procurement->status === 'rejected' ? 'Ditolak' : 'Menunggu')); ?>
                                ...
                                <?php endif; ?>
                                <?php if($procurement->status === 'approved' && $procurement->approver?->name): ?>
                                    | Disetujui oleh <?php echo e($procurement->approver->name); ?>

                                <?php elseif($procurement->status === 'rejected' && $procurement->rejector?->name): ?>
                                    | Ditolak oleh <?php echo e($procurement->rejector->name); ?>
...
<aside id="bookDrawer" class="book-drawer" aria-hidden="true">
    ...
    <div class="book-drawer-body">
        <form id="editBookForm" method="POST" action="" enctype="multipart/form-data" class="space-y-3" data-async="true" data-success-call="closeBookDrawer" data-refresh-targets="#bookStatsWrap,#bookListWrap">
            ...
            <div class="book-grid">
                <input id="drawerBookYear" name="published_year" type="number" min="1901" max="<?php echo e(now()->addYear()->format('Y')); ?>" class="form-input px-3 py-3 text-sm" placeholder="Tahun terbit">
...
<script>
    ...
    function openBookDrawer(button) {
        editBookForm.action = "<?php echo e(url('/admin/books')); ?>/" + button.dataset.id;
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\HP\Downloads\laravel\perpustakaan sekolah\perpus\resources\views\admin\books\index.blade.php ENDPATH**/ ?>