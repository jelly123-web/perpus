...
<div class="member-page">
    <div class="member-toolbar">
        ...
        <div class="member-badge" style="background:var(--accent);color:#fff;padding:8px 16px;border-radius:999px;font-size:12px;font-weight:800;display:flex;align-items:center;gap:8px;"><i data-lucide="tags" class="w-4 h-4"></i> <?php echo e($categories->total()); ?> Kategori</div>
    </div>
    ...
    <section id="categoryStats" class="member-mini-stats">
        <div class="member-mini-stat">
            ...
            <div><div class="member-mini-value"><?php echo e($categories->total()); ?></div><div class="member-mini-label">Total Kategori</div></div>
        </div>
        <div class="member-mini-stat">
            <div class="member-mini-icon" style="background:var(--teal-light);color:var(--teal);"><i data-lucide="book" class="w-4 h-4"></i></div>
            <div><div class="member-mini-value"><?php echo e($categories->sum('books_count')); ?></div><div class="member-mini-label">Buku Tercatat</div></div>
        </div>
        <div class="member-mini-stat">
            <div class="member-mini-icon" style="background:var(--gold-light);color:var(--gold);"><i data-lucide="check-circle" class="w-4 h-4"></i></div>
            <div><div class="member-mini-value"><?php echo e($categories->where('books_count', '>', 0)->count()); ?></div><div class="member-mini-label">Kategori Aktif</div></div>
        </div>
        <div class="member-mini-stat">
            <div class="member-mini-icon" style="background:var(--red-light);color:var(--red);"><i data-lucide="alert-circle" class="w-4 h-4"></i></div>
            <div><div class="member-mini-value"><?php echo e($categories->where('books_count', 0)->count()); ?></div><div class="member-mini-label">Kategori Kosong</div></div>
        </div>
    </section>
    ...
    <div class="cat-shell">
        <div class="cat-add">
            ...
            <form method="POST" action="<?php echo e(route('admin.categories.store')); ?>" class="space-y-5" data-async="true" data-reset-on-success="true" data-refresh-targets="#categoryStats,#categoryList">
                <?php echo csrf_field(); ?>
        ...
        <div id="categoryList" class="cat-card">
            ...
            <div class="grid md:grid-cols-2 gap-4">
                <?php $__empty_1 = true; $__currentLoopData = $categories; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $category): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); $__empty_1 = false; ?>
                    <div class="cat-row">
                        <form method="POST" action="<?php echo e(route('admin.categories.update', $category)); ?>" class="space-y-4" data-async="true" data-refresh-targets="#categoryStats,#categoryList">
                            <?php echo csrf_field(); ?>
                            <?php echo method_field('PUT'); ?>
                            <div class="space-y-1.5">
                                <input name="name" value="<?php echo e($category->name); ?>" class="form-input w-full px-3 py-2.5 text-sm rounded-xl font-bold bg-white" placeholder="Nama kategori">
                            </div>
                            <div class="space-y-1.5">
                                <textarea name="description" class="form-textarea w-full px-3 py-2.5 text-sm rounded-xl bg-white" rows="2" placeholder="Deskripsi"><?php echo e($category->description); ?></textarea>
                            </div>
                            <div class="cat-actions">
                                <span class="text-[11px] font-bold text-slate-400 bg-white px-3 py-1 rounded-full border border-slate-100 mr-auto">
                                    <?php echo e($category->books_count); ?> buku
                                </span>
                                <button type="submit" class="btn-cat-glow primary" style="padding: 8px 14px;">Update</button>
                        </form>
                                <form method="POST" action="<?php echo e(route('admin.categories.destroy', $category)); ?>" data-async="true" data-confirm="Hapus kategori ini?" data-remove-closest=".cat-row" data-refresh-targets="#categoryStats,#categoryList">
                                    <?php echo csrf_field(); ?>
...
<?php echo $__env->make('layouts.admin', array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?><?php /**PATH C:\Users\HP\Downloads\laravel\perpustakaan sekolah\perpus\resources\views\admin\categories\index.blade.php ENDPATH**/ ?>